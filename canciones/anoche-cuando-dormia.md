# Anoche cuando dormía

## Metadatos

### Notion DB

- **Título de la canción:** Anoche cuando dormía
- **Género:** 
- **Tipo:** Canción
- **Año:** 2026
- **Fecha de composición:** 2026-03-02
- **Estado de publicación:** Sin procesar
- **Generador:** 
- **Temas:** Reflexión, Alegría, Esperanza
- **Distribuidor:** 
- **ISRC:** 
- **Álbum:** 
- **NotionPageID:** 5d3a6d10-b693-43b1-9d9d-f54922f6b2f0
- **Música:**

### Producción musical

- **BPM:**
- **Compás:**
- **Tonalidad:**
- **Progresión:**
- **Estructura:**

## Armonía

- **Progresión base:**
- **Patrón rítmico:**
- **Dinámica por sección:**
- **Riff melódico:**

### Acordes por sección

| Sección | Acordes | Notas |
|---------|---------|-------|

## Descripción

Antonio Machado escribió cuatro estrofas que abren con el mismo verso — «Anoche cuando dormía, soñé, ¡bendita ilusión!» — y cada una revela una dimensión distinta del soñar: la fe, el amor, el agua que da vida, el calor del corazón. Musicalizar un poema sin alterar una sola palabra es un acto de artesanía casi monástica. La canción respeta cada sílaba del poeta y construye un arco dramático alrededor de la repetición, transformando el mantra en un viaje que despierta a otra realidad.

## Style Prompt

```

```

---

## Letra

Razonamiento del estilo:
El reto de musicalizar este poema sin alterar su letra es estructural: Machado escribió cuatro estrofas con el mismo patrón de apertura («Anoche cuando dormía / soñé ¡bendita ilusión!»), lo que genera naturalmente una forma estribillo–verso que SUNO puede leer de dos maneras:
Se eligió la opción 2 para maximizar el arco dramático:
Si SUNO corta antes del Final Chorus, usa Continue from this moment con:

---

## Esquema de rima

Poema de Antonio Machado: romance octosilábico (ABCB asonante en versos pares).

## Checklist Anti-AI

| # | Safeguard | Cumple |
|---|-----------|--------|
| 1 | 30%+ rimas asonantes/libres | ✅ poema de antonio machado: romance octosilábico (abcb asonante en versos pares). |
| 2 | Máximo 1 tríada | ✅ 0 tríadas |
| 3 | 0 em dashes | ✅ |
| 4 | ≥1 coloquialismo por estrofa | ⚠️ 0 coloquialismos para 1 estrofas |
| 5 | ≥1 verso métrica quebrada | ✅ (asumido — validación manual requerida) |
| 6 | No etiquetar figuras retóricas | ✅ |
| 7 | Detalles sensoriales ≥1 por estrofa | ✅ musica |
| 8 | Verbos/adj ≥ 2:1 | ✅ (asumido — validación manual requerida) |
| 9 | 1 imagen absurda o surrealista | ✅ (asumido — validación manual requerida) |
| 10 | Cero palabras listado prohibido | ✅ |
| 11 | Cero AI-ismos semánticos | ✅ |
| 12 | Cero verbos forzados | ✅ |
| 13 | Cero negative parallelism | ✅ |
| 14 | Sin parallel negation encadenada | ✅ |
| 15 | Sin anaphora abuse (máx 2 versos) | ✅ (máx 0 versos consecutivos) |
| 16 | Puente sin "Pero" al inicio | ✅ (sin puente o sin "Pero") |
| 17 | Título repetido <4 veces en chorus | ✅ (0 veces en coro) |
| 18 | Abstracto anclado a objeto concreto | ✅ (asumido — validación manual requerida) |
| 19 | Especificidad objetual | ⚠️ 0 objetos concretos para 1 estrofas |
| 20 | Sin promoción alcohol/tabaco/drogas | ✅ |
| 21 | Principios edificantes | ✅ |

## Changelog de Autoría
